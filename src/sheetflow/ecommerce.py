...
def normalize_frame(frame: pd.DataFrame, mapping: dict[str, str]) -> pd.DataFrame:
    missing = [
        name for name in ("order_id", "shop_name", "quantity", "amount") if name not in mapping
    ]
    if missing:
...
def run_ecommerce(config: WorkflowConfig, mapping: dict[str, str] | None = None) -> TaskReport:
    ...
    summary_cfg = OutputConfig(
        path=str(output.with_name("订单汇总.xlsx")), format="xlsx", overwrite=True
    )
